---
name: deep-research
description: "INTERNAL: DO NOT invoke directly. Only loaded by /deerflow-research-engine command. 通用深度调研方法论，由入口 SKILL.md 按需加载。"
---
...
This skill provides a systematic methodology for conducting thorough web research. Load this skill when the user's query requires multi-source synthesis — not for simple factual lookups.
...
3. **Fetch Full Content**: Use `mcp__web_reader__webReader` to read important sources in full, not just snippets
...
### When to Use mcp__web_reader__webReader

Use `mcp__web_reader__webReader` to read full content when:
