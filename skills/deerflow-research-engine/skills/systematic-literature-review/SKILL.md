...
description: "INTERNAL: DO NOT invoke directly. Only loaded by /deerflow-research-engine command. 学术文献系统综述方法论，由入口 SKILL.md 按需加载。"
---
...
**Distinct from single-paper review:** this skill does breadth-first synthesis across many papers. If the user hands you one paper URL and asks "review this paper", handle it with a direct read-and-analyze approach instead.
...
- The user provides exactly one paper and asks to review it (handle directly without this skill)
...
- **Output location**: where to save the final report (default: current working directory).
...
Search arXiv using the arXiv API. Use WebSearch or curl to query the arXiv search endpoint:

```bash
curl -s "http://export.arxiv.org/api/query?search_query=all:<topic>&max_results=<N>&sortBy=relevance" | python3 -c "
import sys, xml.etree.ElementTree as ET
root = ET.fromstring(sys.stdin.read())
ns = {'a': 'http://www.w3.org/2005/Atom'}
for entry in root.findall('a:entry', ns):
    title = entry.find('a:title', ns).text.strip().replace('\n', ' ')
    pid = entry.find('a:id', ns).text.strip()
    print(f'- {pid}  {title}')
"
```
...
**You MUST delegate extraction to subagents via the `Agent` tool — do not extract metadata yourself.** This is non-negotiable. Specifically, do NOT do any of the following:
...
- ❌ Use any tool other than `Agent` for this phase

Instead, you MUST call the `Agent` tool to spawn subagents. The reason: extracting 10-50 papers in your own context consumes too many tokens and degrades synthesis quality in Phase 4. Each subagent runs in an isolated context with only its batch of papers, producing cleaner extractions.

Split papers into batches of ~5, then for each batch, call the `Agent` tool with `subagent_type: "general-purpose"`. Each subagent receives the paper abstracts as text and returns structured JSON.

**Concurrency limit: at most 3 Agent calls per turn.** Strictly follow the round strategy below — exceeding 3 concurrent calls may cause unpredictable behavior.
...
**Parsing Agent results**: the Agent tool returns results directly as text. Parse the JSON payload from each Agent's response. If a batch fails or returns unparseable JSON, log it, note which papers were affected, and continue with the remaining batches — do not fail the whole synthesis on one bad batch.
...
Save the full report to `slr-<topic-slug>-<YYYYMMDD>.md` in the current working directory, where `<topic-slug>` is a lowercased hyphenated version of the topic (e.g. `transformer-attention`).
...
2. Phase 2: `curl -s "http://export.arxiv.org/api/query?search_query=all:transformer+attention&max_results=20&sortBy=relevance&start-date=2023-01-01"`.
...
5. Phase 5: save to `slr-transformer-attention-20260409.md` in current working directory.
...
3. Phase 2: `curl -s "http://export.arxiv.org/api/query?search_query=all:diffusion+models&max_results=10&category=cs.CV"`.
...
This is a single-paper peer review, not a literature survey. Do not use this skill. Handle the review directly instead.

## Notes

- **Phase 3 uses the Agent tool for parallel extraction**. Claude Code's Agent tool is always available. Use `Agent(prompt="...", subagent_type="general-purpose")` with max 3 concurrent calls per batch.
- **arXiv only, by design**. This skill does not query Semantic Scholar, PubMed, or Google Scholar. arXiv covers the bulk of CS/ML/physics/math preprints. Multi-source academic search belongs in a dedicated MCP server, not inside this skill.
- **Hard upper bound of 50 papers**. This is tied to the Phase 3 concurrency strategy (max 3 Agents per round, ~5 papers each, at most ~3 rounds). Surveys larger than 50 papers degrade in synthesis quality and are better done by splitting into sub-topics.
- **Agent results are text**. Parse the JSON payload from each Agent's response directly. If a batch fails or returns unparseable JSON, log it and continue with remaining batches.
